...
<div id="user" class="main_container wallpaper2">
	...
	<div class="right_area">
		...
		<div class="order" style="margin:0;">
			<div class="title">
				我的订单
				<div class="count">共有<?php echo ($orderCount); ?>份订单</div>
			</div>
			<table>
				...
				<tr>
					...
					<td><a href="<?php echo U('Line/show?id='.$order['line_id']); ?>" target="_blank"><?php echo ($order["line_name"]); ?>&nbsp;×&nbsp;<?php echo ($order["people_count"]); if ($order['type'] == 4) echo '&nbsp;张'; else echo '&nbsp;人'; ?></a></td>
					<?php } else { ?>
			...
			<div class="pagination">
				...
				<ul>
					<?php if ($cntPage >= 5) { ?>
					<li><a href="<?php echo U('User/order?page=1') ?>">1</a></li>
					<?php } ?>
					<?php if ($cntPage > 5) { ?>
					<li class="disable"><a>...</a></li>
					<?php } ?>
					<?php for($i = $cntPage-3; $i < $cntPage; $i++) { if ($i <= 0) continue; ?>
					<li><a href="<?php echo U('User/order?page='.$i) ?>"><?php echo ($i); ?></a></li>
					<?php } ?>
					...
					<?php } ?>
					<?php if ($cntPage <= $totPage-4) { ?>
					<li><a href="#"><?php echo ($totPage); ?></a></li>
					<?php } ?>
				</ul>
			</div>
		</div>
